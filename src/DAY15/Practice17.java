...
-------------------------------------------*/
        ...
        stringBox.content="안녕하세요";
        System.out.println("[문자]"+stringBox.content);

        Box<Integer> integerBox = new Box<>();
        integerBox.content=100;
        System.out.println("[숫자]"+integerBox.content);
...
-------------------------------------------*/
        ...
        treeSet.addAll(lottoSet); // 다른집합의 요소들을 새로운 요소에 대입 , addAll(집합객체)
        System.out.println(treeSet.descendingSet()); // 내림차순
...
-------------------------------------------*/
        ...
        // 배열은 프린트찍으면 주소값 나옴
        System.out.println(Arrays.toString(jsonAry)); // 쉼표가 2개이므로 3조각 반환 
        
        for(int i=0; i<=jsonAry.length-1; i++){
            String str = jsonAry[i];
            System.out.println("str = " + str);
            System.out.println();
            String [] strAry = str.split(":");
            System.out.println(Arrays.toString(strAry));
            // " 큰따음표 제거
            jsonMap.put(strAry[0].replace("\"",""),strAry[1]);
        }
        System.out.println(jsonMap);
        System.out.println("jsonMap = " + jsonMap.get("name"));
        System.out.println("jsonMap = " + jsonMap.get("age"));
...
-------------------------------------------*/
        ...
        //주의 : new Dto1개 == new Map 1개 , .put 1개 == 속성 1개
        Map<String , Object>map1 = new HashMap<>();
        map1.put("name","삼성전자");
        map1.put("price","72000");
        map1.put("volume","1500000");
        stockList.add(map1);

        Map<String , Object>map2 = new HashMap<>();
        map2.put("name","카카오");
        map2.put("price","52000");
        map2.put("volume","800000");
        stockList.add(map2);

        Map<String , Object>map3 = new HashMap<>();
        map3.put("name","네이버");
        map3.put("price","210000");
        map3.put("volume","300000");
        stockList.add(map3);
...
class Box<T>{ //<제네릭타입> 이란 클래스 작성시 미리 타입 정하지 않고 인스턴스 생성시 타입 정한다.
    public T content;
}