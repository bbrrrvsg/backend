...
#[1] INSERT : 레코드 / 행 / 튜플 / 1줄 /*DTO 1개  삽입 
...
#5) not null  제약조건 꼭 삽입 해야한다. 
-- insert into test(mno,mcount) values(5,25); -- 오류발생
#6) unique 제약조건 속성은 중복값이 불가능 하다.
-- insert into test(mname) values("유재석"); -- 오류발생 중복 
#7) 모든속성에 대해 값을 삽입할 경우 속성명 생략 가능하다. 값 대응시 속성명 순서 일치 
insert into test values(5,"박명수",60); 
#8) 하나의 명령어에서 다수 레코드 삽입 values(레코드1) , (레코드2) ,(레코드3);
insert into test values(6,"하하",70) , (7,"바바나",80);
...
#3) select 속성명 from 테이브령 where 조건절 
select * from test where mname ="유재석"; -- SQL 에서는 = 같다 기호로 사용 
select*from test where mcount>=40; 


-- [3] UPDATE , 레코드 값 수정 , mysql workbench safeMode해제 : 
set SQL_SAFE_UPDATES = 1; -- 활성화
set SQL_SAFE_UPDATES = 0; -- 수정 과 삭제는 위험하므로 workbench에서는 수정.삭제 차단 상태 

#1) update 테이블명 set 수정할속성명 = 수정할값 , 수정할속성명 = 수정할값 where 조건 
update test set mcount =0; -- where 조건이 없으면 전체 수정 

#2) 일반적으로 조건 수정이다.
update test set mcount=10 where mname = "유재석"; -- mname이 유재석이면 mcount 10 
update test set mcount=20 , mname ="강호동2" where mno =2;  -- mno 속성값이 2이면 mcount 속성값 변경

-- [4] delete , 레코드 삭제 , delete : 레코드 삭제 vs truncate: 모든레코드삭제 vs drop : 테이블/데이터베이스 삭제 
#1) delete from 테이블명 where 조건 
delete from test where mno =2; -- 강호동 레코드 삭제 
...
/*
	DDL : 데이터베이스 정의어 , 뒤로가기(롤백) 불가능 
		create , drop , truncate , show
    DML : 데이터베이스 조작어 , 뒤로가기(롤백) 가능 
		insert , select , update , delete 
*/ 

# 기본 연산자
#1) 산술 연산자 : + , - , * , /     div 몫 mod나머지 
...
#1] 별칭 키워드 , 조회결과의 속성명 변경 , as 생략 가능하다. , 속성명 / 테이블명 같거나 복잡한 경우 사용. 
...
#3] 산술연산자 , 주의할점 : select 결과는 항상 표/테이블!!
select mnumber , mnumber + 3 as 더하기 , mnumber-3 as 뺴기 , mnumber/3 as 나누기  , mnumber * 3 as 곱하기 ,mnumber div 3 as 몫 , mnumber mod 3 as 나머지 from member; 
...
select * from member where maddr in ('경기' , '전남' , '경남'); -- 속성명 in (값,값,값) 
# 주의 할점 : null(자료없다 뜻) 비교연사자 별도 존재
...
select * from member where manme like "에이_"; -- mname 속성값이 '에이'로 시작하면서 3글자 문자 
select * from member where mname like '%핑그'; -- '핑크'로 끝나는 문자 
select * from member where mname like '%이%'; -- '이'가 포함된 문자 
select * from member where mname like '_이%'; -- 두번쨰 글자가 '이'인 문자 
