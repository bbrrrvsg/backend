drop database if exists mydb2010;
create database mydb2010;
use mydb2010;
...
# 관계형데이터베이스(RDMS)는 정규화(테이블 나는 과정) 인한 테이블이 분해 과정 
# 조회(SELECT) 하는 과정에서 다시 테이블들을 합치기(생성x 조회o) 
# 즉] 정규화(테이블 나누기) 가 많아지면 조회가 복잡하다. vs 반정규화(최소한 중복허용) 조회 복잡도가 간소화

# (1) 두개 이상의 테이블 같이 조회하기 

-- table1(5개레코드) table2(5개레코드) 조회시 10개가 아닌 25개 조회된다.
-- 주의할점 : 두 테이블 이상의 조회시 데카르트 곱으로 표현한다. 
select*from table1  , table2; -- 25개(하나의 레코드가 다른테이블의 모든 레코드 표현) 

# (2) 교집합 : 두테이블 간의 공통된 값 기준으로 레코드 조회 
# 2-1) where 절 이용한 pk속성값과 fk속성 같으면 , -- 일반 조건과 구분이 어렵기때문에 비권장 
...
# 2-4) natural join 절 , 전제조건 : pk 와 fk 속성명 같다는 조건 
select*from table1 natural join table2 t2;  -- 조인 조건 생략 가능 
...
# (3) 외부 조인 : OUTER JOIN 절 , OUTER 생략가능 
# 3-1) 왼쪽 테이블의 모든 레코드 조회 + 오른쪽 테이블의 교집합 레코드 조회 없으면 null로 표현 
...
# (4) 합집합 , MYSQL : UNION(중복레코드 제외한 레코드 합치기) /  ORACLE : FULL OUTER JOIN  다르다 
...
# 오른쪽 테이블의 차집합 
select*from table1 t1 right join table2 t on t1.num_pk=t2.num_fk where t1.num_pk is null;
