...
#[2] 집계 함수 
select sum(bamount) from buy; 	-- buy 테이블 내에 bamount 속성값들의 전체 합계 
select avg(bamount) from buy; 	-- 평군   
select min(bamount) from buy; 	-- 최솟값 
select max(bamount) from buy; 	-- 최댓값
select count(bamount) from buy; -- 전체 레코드 수 (null 제외) 
select count(*) from buy; 		-- 전체 레코드 수 (null 포함) 

#[3] 그룹절과 집계함수 
#1) 회원아이디별 구매수량 합계 
select*from buy;					-- 전체 조회 
select mid from buy group by mid;	-- mid 기준 
select mid,sum(bamount) from buy group by mid;	-- mid 기준 bamount 총합계 

#2) 회원아이디별 총 구매금액(구매수량 * 구매가격) 
select mid , sum(bamount * bprice) from buy group by mid; -- mid 별로 구매수량과 구매가격을 곱하기한 총구매금액 

#3) 총 판매 횟수 , 회원아이디 별로 판매 횟수 
select count(*) from buy; 
select mid,count(*) from buy group by mid; 

#[4] having : 그룹절의 조건절 , where : 그룹 전 조건절 vs having : 그룹후 조건 
#1) 그룹 전 조건  where , 구매수량(그룹전에 존재하는 속성) 이 3이상인
select * from buy where bamount > 3 ; 
#2) 그룹 후 조건 having , 총 구매금액 (그룹후에 존재하는 속성)이 10000이상인
select mid , sum(bamount*bprice)
 as 총구매금액 from buy group by mid having 총구매금액>1000;
 
 # 작성 순서 -- select 속성명 from 테이블명 where 일반조건절 group by 그룹기준 having 그룹 조건절 
 
 #[5] order by : 정렬 , asc 오름차순(기본값)  , desc 내림차순 
 # 주의할점 : 다중 정렬은 order by 1차 , 2차 
 ...
 select * from member limit 0,2; 	-- 조회결과 0(첫번째레코드) 부터 2개 조회 한다. 
 select * from member limit 0,5;    -- 1페이지, 5개 조회 
 select * from member limit 5,5;    -- 2페이지, 5개 조회 
 
 # select 작성 규칙/순서 
 # select 속성명 from 테이블명 where 일반조건 group by 그룹기준 order by 정렬기준 limit 개수 
 # select 처리 순서 
