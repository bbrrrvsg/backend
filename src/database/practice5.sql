...
select p.제품명,pc.카테고리명,s.재고수량 from pcategory pc inner join product p on p.카테고리번호_fk=pc.카테고리번호_pk inner join stock s on p.제품번호_pk=s.제품번호_fk;
...
select pc.카테고리명 , sum(s.재고수량) as 총재고수량 from pcategory pc inner join product p on pc.카테고리번호_pk = p.카테고리번호_fk inner join stock s on p.제품번호_pk=s.제품번호_fk group by pc.카테고리명;
-- [문제 9] 각 제품별로 총 재고 수량을 조회하고, 총 재고 수량이 많은 순서대로 정렬하여 제품명과 총재고수량을 표시하세요. (조회결과: 6개 레코드)
select p.제품명 , sum(s.재고수량) as 총재고수량 from product p inner join stock s on p.제품번호_pk=s.제품번호_fk group by p.제품명 order by 총재고수량 DESC;
