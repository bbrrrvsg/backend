...
select distinct customer from orders group by customer; 
...
select distinct author from books group by author;  
...
select customer, count(order_qty) as 주문건수 from orders group by customer; 
...
select customer , sum(order_qty*price) as 총구매금액 from books,orders group by customer;  
