...
public class Practice1 {
    public static void main(String[] args) {
        ...
        //1.
        // 리터럴 : 개발언어 에서 미리 정해진 값들을 = 상수
        // sout 엔터
        System.out.println(25);
        System.out.println("홍길동");

        //2. 변수 : 하나늬 자료를 저장하는 메모리 공간
        // 타입 변수명 = 값/리터럴값/객체/배열(함수는 못늠)
        int age = 10;
        System.out.println(age); // "age" 는 문자열 vs age 는 키워드(변수명/함수명/if/for/int등)는 프로그램이(기능포함) 이해하는 단어

        //3. 변수는 동일한 {} 내에 중복이름 불가
        String name ="유재석"; int age1 = 10;
        double height = 180.2; String motto = "좌우명";

        //4. + 기호는 2사지 기능 갖는다.  (1) +더하기 (2) 연결 : 숫자+문자
        System.out.println("제이름은"+ name+ "나이는"+age1+"키는"+height+"cm 입니다");

        //5. 형식 printf , 문자열내 복잡한 수식이나 변수가 들어가는 자리에 형식으로 표현
        // %d : 정수 %s : 문자 %f : 실수 %c : 문자

        System.out.printf("제이름은 %s ,나이는 %d , 키는 %f cm입니다. " , name ,age1 , height );
        ...
        // 주의할점 nextLine() 앞에 다른 next() 존재하면ㄴ nextLine() 필요하다
        System.out.print("게시물 번호 : "); int bno = scan.nextInt(); scan.nextLine();
        System.out.print("제목 : "); String title = scan.nextLine();
        System.out.print("내용 : "); String comment = scan.nextLine();
        System.out.printf("%d번 게시물",bno);
        System.out.printf("제목 : %s",title);
        System.out.printf("내용 : %s " , comment);

        //10. 문자 입력 받기 next().charAt();
        System.out.print("성별 : "); char 성별 = scan.next().charAt(0);
        System.out.printf("입력하신 성별은 %s 입니다 ",성별);

        //11.
        System.out.print("이름 : "); String 성함 = scan.next();
        System.out.print("나이 : "); int 연령 = scan.nextInt();
        System.out.print("키 : "); double 신장 = scan.nextDouble();
        System.out.print("프로그래머 여부  : "); boolean 여부 = scan.nextBoolean();
