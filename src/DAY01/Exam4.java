...
public class Exam4 {
    public static void main(String[] args) {
        ...
        // new Scanner : Scanner 객체 만들곘다
        // (System.in) : 시스템 입력 갳체를 Scanner 객체에게 전달
        // * 시스템 입력 객체를 Scanner 객체에 대입하여 Scanner객체 생성
        ...
        String str1 = 입력변수.next();
        System.out.println(str1);

        // 2. .nextLine();  : 입력받은 자료를 문자열로 반환 , 띄어 쓰기 포함
        // 주의할점 : 띄어쓰기를 포함하다 보니 앞전에 next() 충돌하여 하나의 next인식된다
        // 해결방법 : nextLine() 사용할때 앞전에 next() 존재하면 임의의 nextLine() 하나더 작성한다.
        입력변수.nextLine(); // 해결
        System.out.print("2. nextLine() : ");
        String str2 = 입력변수.nextLine();
        System.out.println(str2);

        // 3. .nextByte() : 입력받은 자료를 바이트로 변환 , -128 ~ 127 이외 자료이면 오류
        System.out.print("3. nextByte() : ");
        byte b1 = 입력변수.nextByte();
        System.out.println(b1);

        //4. .nextShort() : 입력받은 자료를 쇼트로 반환 . +- 3만정도 이외자료 오류
        System.out.print("4. .nextShort() : ");
        short s1 = 입력변수.nextShort();
        System.out.println(s1);

        //5. .nextInt() : 입력받은 자료를 인트로 반환
