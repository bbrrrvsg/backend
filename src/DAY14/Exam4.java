    ...
    public static void main(String[] args) {
        /*
            Thread Pool
                ...
                콜 요청  --->    직원2(강호동/스레드)
                                직원3(신동엽/스레드)
                만약에 2개 콜이 동시에 들어오면 2명이 각 배정 되고 직원 1명은 대기
                만약에 5개 콜이 동시에 들어오면 3명이 각 배정 되고 콜요청 2개 대기
                6. 사용처 : 1) 서버프로그램(톰캣) 2) 채팅 3) 대기프로그램 4) SPRING 프레임워크
                7. 사용법
                    1) ExecutorService executorService = Executors.newFixedThreadPool(총 스레드수 );
                    2) executorService.submit(스레드객체);
         */
        //[1] 작업스레드풀 배정/만들기 , 3= 총 3개의 스레드가 돌아가면서 작업 처리한다.
        ExecutorService executorService = Executors.newFixedThreadPool(3);
        //[2] 여러 스레드르 만들어서 각 스레드 들을 작업스레드풀에 등록하기
        for(int i=1; i<=10; i++){
            String name = "Thread"+i; // 스레드 이름 배정
            //[3] 작업 스레드 생성
            Runnable runnable = new Runnable() {
                @Override public void run() {
                    try{Thread.sleep((1000 + new Random().nextInt(1000)));}catch (Exception e){}
        ...
        //[5] 스레드풀 종료 (스레드 풀에 저장된 모든 스레드 안전하게 종료)
